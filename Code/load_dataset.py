...
def get_cross_dataloader(data_args):
    ...
    for k_fold_1, (train_val_idx, test_idx) in enumerate(
        skf_1.split(dataset.input, dataset.target)
    ):
        train_val_dataset = dataset.iloc[train_val_idx]
        test_dataset = dataset.iloc[test_idx]
        test_d = np.average(test_idx)

        for k_fold_2, (train_idx, val_idx) in enumerate(
            skf_2.split(train_val_dataset.input, train_val_dataset.target)
        ):
        ...
        dataloader["train"] = DataLoader(
            InputDataset(train_dataset), batch_size=data_args.batch_size, shuffle=True
        )
        dataloader["valid"] = DataLoader(
            InputDataset(val_dataset), batch_size=data_args.batch_size, shuffle=False
        )
        dataloader["test"] = DataLoader(
            InputDataset(test_dataset), batch_size=data_args.batch_size, shuffle=False
        )

        dataloader_list.append(dataloader)

        dataloader_full = dict()
        dataloader_full["train"] = DataLoader(
            RawDataset(train_dataset), batch_size=data_args.batch_size, shuffle=True
        )
        dataloader_full["valid"] = DataLoader(
            RawDataset(val_dataset), batch_size=data_args.batch_size, shuffle=False
        )
        dataloader_full["test"] = DataLoader(
            RawDataset(test_dataset), batch_size=data_args.batch_size, shuffle=False
        )
...
def get_dataloader(data_args):
    ...
    train_r, valid_r, test_r = random_split(
        read,
        lengths=[
            int(data_split_ratio[0] * len(read)),
    ...
    train_n, valid_n, test_n = random_split(
        neutral,
        lengths=[
            int(data_split_ratio[0] * len(neutral)),
    ...
    train_u, valid_u, test_u = random_split(
        unread,
        lengths=[
            int(data_split_ratio[0] * len(unread)),

...
from explanation.utils.shapley import (
    GnnNetsGC2valueFunc,
    GnnNetsNC2valueFunc,
    NC_mc_l_shapley,
    gnn_score,
...
class PlotUtils(object):
    ...
    def plot(self, graph, nodelist, figname, title_sentence=None, **kwargs):
        ...
        if self.dataset_name.lower() in ["ba_2motifs", "ba_lrp"]:
            ...
            self.plot_code_readability(
                graph, nodelist, x, data_raw, edgelist=edgelist, figname=figname
            )
        else:
            raise NotImplementedError
    ...
    def plot_code_readability(
        self, graph, nodelist, x, data_raw, edgelist=None, figname=None
    ):
        ...
        subgraph_edge_color = [
            sub_edge_colors[all_edge_types.index(v)] for v in edges_types
        ]
        ...
        type_labels = (
            loop_type + branch_type + exception_type + other_type + declaration_type
        )
...
class SubgraphX(object):
    ...
    #            print(f"MCTS execution time: {time.time() - t_mcts1:.4f}s")
            ...
            self.mcts_state_map = self.get_mcts_class(
                data.x, data.edge_index, node_idx=node_idx
            )
    ...
    #            print(f"MCTS execution time: {time.time() - t_mcts:.4f}s")
        ...
        fidelity_result = fidelity(
            prob.squeeze()[prediction], torch.tensor(maskout_score)
        )
        related_pred = {
            "masked": masked_score,
    ...
    def __call__(self, data, **kwargs) -> Tuple[None, List, List[Dict]]:
